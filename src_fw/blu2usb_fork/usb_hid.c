...
bool usb_hid_submit_mouse(const usb_mouse_report_t *report) {
    if (report == NULL || g_release_mouse_pending) return false;
    if (tud_suspended()) {
        tud_remote_wakeup();
        return false;
    }
    if (!tud_hid_n_ready(BLU2USB_USB_HID_MOUSE_INTERFACE)) return false;
    return tud_hid_n_report(BLU2USB_USB_HID_MOUSE_INTERFACE, 0u,
                            report, (uint16_t)sizeof(*report));
}
...
void usb_hid_task(void) {
    ...
    if (g_release_mouse_pending && tud_hid_n_ready(BLU2USB_USB_HID_MOUSE_INTERFACE)) {
        const usb_mouse_report_t neutral = {0};
        if (tud_hid_n_report(BLU2USB_USB_HID_MOUSE_INTERFACE, 0u,
                             &neutral, (uint16_t)sizeof(neutral)))
            g_release_mouse_pending = false;
    }
    if (tud_hid_n_ready(BLU2USB_USB_HID_KEYBOARD_INTERFACE)) {
        ...
        if (keyboard_report_queue_peek(&g_keyboard_queue, &report) &&
            tud_hid_n_report(BLU2USB_USB_HID_KEYBOARD_INTERFACE, 0u,
                             &report, (uint16_t)sizeof(report)))
